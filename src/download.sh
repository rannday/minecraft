...
SERVER_DIR="${SERVER_DIR:?SERVER_DIR not set by env.sh}"
...
[[ -n "$latest_version" && -n "$version_url" ]] || {
...
if [[ -L "$link_path" ]]; then
  current_target=$(resolve_symlink "$link_path")
  if [[ "$current_target" == "$jar_path" ]]; then
    ...
    sudo -u "$MC_USER" ln -sf "$jar_path" "$link_path"
...
echo "Done."
echo "JAR:      $jar_path"
resolved_target="$(resolve_symlink "$link_path" 2>/dev/null || echo "unresolved")"
echo "Symlink:  $link_path -> $resolved_target"
